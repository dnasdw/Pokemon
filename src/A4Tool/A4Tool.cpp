...
int unpackA4(const UChar* a_pA4FileName, const UChar* a_pArcFileNamePrefix)
{
	FILE* fp = UFopen(a_pA4FileName, USTR("rb"), false);
	if (fp == nullptr)
	{
		return 1;
	}
	fseek(fp, 0, SEEK_END);
	u32 uA4Size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	u8* pA4 = new u8[uA4Size];
	fread(pA4, 1, uA4Size, fp);
	fclose(fp);
...
int packA4(const UChar* a_pA4FileName, const UChar* a_pArcFileNamePrefix)
{
	return 0;
}
...
int UMain(int argc, UChar* argv[])
{
	...
	if (UCslen(argv[1]) == 1)
	{
		...
		case USTR('U'):
		case USTR('u'):
			return unpackA4(argv[2], argv[3]);
		case USTR('P'):
		case USTR('p'):
			return packA4(argv[2], argv[3]);
		default:
			break;
		}
	}
